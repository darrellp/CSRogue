...
namespace CSRogue.Map_Generation
{
	...
	{
        #region Scanning
        // This scanning modified from Adam Milazzo's code at this page: http://www.adammil.net/blog/v125_roguelike_vision_algorithms.html
        // I modified it SLIGHTLY to work in my context and it dropped in like a charm.
        public void Compute(MapCoordinates origin, int rangeLimit)
        {
            SetVisible(origin.Column, origin.Row);
        ...
        void Compute(int octant, MapCoordinates origin, int rangeLimit, int x, Slope top, Slope bottom)
        {
            ...
            for (; x <= rangeLimit; x++) // (x <= (int)rangeLimit) == (rangeLimit < 0 || x <= rangeLimit)
            {
                ...
                int rangeLimitSq = rangeLimit * rangeLimit;
        ...
        {
            bool blocked;
            return IsOpaqueAt(_map, new MapCoordinates(x, y), out blocked);
        }

        void SetVisible(int x, int y)
        {
            _currentFOV.Add(new MapCoordinates(x, y));
        }
